# -*- coding: utf-8 -*-
"""
SignalScape - TDMS数据可视化分析工具 (PySide6 版本)
...
from PySide6.QtGui import (
    QAction, QColor, QFont, QIcon, QPainter, QPen, QBrush,
...
QPushButton:pressed { background-color: #2d4fd8; }
QPushButton:disabled { background-color: #b0b8c8; color: #d0d0d0; }
QPushButton#secondary { background-color: #e2e8f0; color: #4a5568; }
...
QToolButton:hover { background-color: #e8f0fe; border-color: #c5d8f7; color: #4a6cf7; }
QToolButton:pressed { background-color: #d0e0fd; }
...
QMenu::item { padding: 4px 20px 4px 10px; border-radius: 3px; font-size: 12px; }
...
class ColorDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        color_hex = index.data(Qt.ItemDataRole.DisplayRole) or "#cccccc"
...
class CheckboxDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        painter.setRenderHint(QPainter.RenderHint.Antialiasing)
        rect = option.rect
        bg = QColor("#e8f0fe") if (option.state & QStyle.StateFlag.State_Selected) else QColor(255, 255, 255)
        painter.fillRect(rect, bg)
        size = 16
        cx = rect.x() + (rect.width() - size) // 2
        cy = rect.y() + (rect.height() - size) // 2
        r = QRect(cx, cy, size, size)
        checked = index.data(Qt.ItemDataRole.DisplayRole) == "1"
        if checked:
...
class LineStyleDelegate(QStyledItemDelegate):
    ...
    def paint(self, painter, option, index):
        ...
        if option.state & QStyle.StateFlag.State_Selected:
            ...
            painter.fillRect(option.rect, QColor(255, 255, 255))
        linestyle = index.data(Qt.ItemDataRole.DisplayRole) or "实线"
        rect = option.rect
        ...
        x2 = x1 + line_len
        style_map = {'实线': Qt.PenStyle.SolidLine, '虚线': Qt.PenStyle.DashLine,
                     '点线': Qt.PenStyle.DotLine, '点划线': Qt.PenStyle.DashDotLine}
        pen = QPen(QColor("#333333"), 1.5, style_map.get(linestyle, Qt.PenStyle.SolidLine))
        painter.setPen(pen)
...
class LineStylePopupWidget(QWidget):
    ...
    def paintEvent(self, event):
        ...
        p.drawRoundedRect(self.rect().adjusted(0, 0, -1, -1), 8, 8)
        # 各项
        style_map = {'-': Qt.PenStyle.SolidLine, '--': Qt.PenStyle.DashLine,
        ...
        for i, (name, code) in enumerate(self._items):
            ...
            p.setPen(QPen(QColor("#333333"), 2,
                          style_map.get(code, Qt.PenStyle.SolidLine)))
    ...
    def paintEvent(self, event):
        ...
        for i, lw in enumerate(self._items):
            ...
            if i == self._hover_idx:
                ...
                p.drawRoundedRect(4, y, self._w - 8, self._item_h - 2, 4, 4)
            w = float(lw)
            p.setPen(QPen(QColor("#333333"), w, Qt.PenStyle.SolidLine))
            ...
            p.drawText(58, y, 50, self._item_h - 2,
                       Qt.AlignmentFlag.AlignVCenter | Qt.AlignmentFlag.AlignLeft, lw)
...
class SaveImageDialog(QDialog):
    ...
    def _create_ui(self):
        ...
        axis_layout.addWidget(QLabel("坐标字体大小:"))
        self.axis_font_size = QSpinBox()
        self.axis_font_size.setRange(6, 24)
        self.axis_font_size.setValue(9)
        self.axis_font_size.setToolTip("坐标轴标签和刻度文字的大小")
        axis_layout.addWidget(self.axis_font_size)
        axis_layout.addStretch()
        layout.addLayout(axis_layout)

        # 图例字体大小
        legend_layout = QHBoxLayout()
        legend_layout.addWidget(QLabel("图例文字大小:"))
        ...
        legend_layout.addWidget(self.legend_font_size)
        ...
        self.preview_btn.setToolTip("将当前字体设置应用到绘图区预览效果")
...
class MatplotlibPlotPanel(QWidget):
    ...
    def _create_toolbar(self):
        ...
        panel.setStyleSheet("""
            ...
            QCheckBox::indicator:checked { background-color: #4a6cf7; border-color: #4a6cf7; }
            QCheckBox::indicator:hover { border-color: #4a6cf7; }
            QPushButton#tool_btn { background-color: #4a6cf7; min-height: 22px; padding: 2px 12px; }
            QPushButton#tool_btn:hover { background-color: #3b5de7; }
        """)
    ...
    def on_save_image(self):
        ...
        result = dlg.exec()
        settings = dlg.get_settings() if result == QDialog.DialogCode.Accepted else None
...
class ParameterGridPanel(QWidget):
    ...
    def _auto_resize_columns(self):
        """按 COL_SIZES 比例分配列宽，使所有列都能在可见区域显示"""
        total_width = self.grid.viewport().width()
        if total_width <= 0:
            return
        # 计算比例总和
        total_ratio = sum(self.COL_SIZES)
        scrollbar_w = 20  # 预留垂直滚动条宽度
        available = total_width - scrollbar_w
        if available <= 0:
            return
        for i, ratio in enumerate(self.COL_SIZES):
    ...
    def _on_context_menu(self, pos):
        ...
        if item:
            row = item.row()
            self._show_cell_menu(row, pos)
        else:
            self._show_blank_area_menu(pos)

    def _show_blank_area_menu(self, pos):
        menu = QMenu(self)
    ...
    def _show_cell_menu(self, row, pos):
    ...
    def add_channel(self, display_name, full_path=None):
        ...
        for col, text in enumerate(items):
            ...
            if col in (2, 3, 4, 5, 6):
                item.setFlags(item.flags() & ~Qt.ItemFlag.ItemIsEditable)
            # 居中对齐
            if col in (1, 6, 8, 9, 10, 11, 12, 13, 14, 15, 16):
...
class UserGuideDialog(QDialog):
    ...
    def _select_section(self, index):
        ...
        self.content_ctrl.setHtml("<div style='font-family: Microsoft YaHei; font-size: 13px; color: #2d3436; line-height: 1.6;'>" + "".join(html_parts) + "</div>")

        # 同步树选中
        if not self._closing:
            root = self.tree.topLevelItem(0)
            if 0 <= index < root.childCount():
                self.tree.setCurrentItem(root.child(index))
...
class SignalScapeMainWindow(QMainWindow):
    ...
    def _on_tree_context_menu(self, pos):
        ...
        if parent is None and item.childCount() > 0:
            ...
            action = menu.exec(self.channel_tree.viewport().mapToGlobal(pos))
