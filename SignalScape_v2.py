...
class DataManager:
    """数据管理类 - 使用pandas DataFrame管理数据"""
    def __init__(self):
        self.files_data = OrderedDict()
        self.df = pd.DataFrame(columns=['file', 'group', 'channel', 'data', 'sample_time'])
        ...
        def target():
            with self._loading_lock:
                ...
                if self._cancel_load:
                    ...
                    if len(self.df) > 0:
                        self.df = self.df[self.df['file'] != original_file_name].reset_index(drop=True)
                    return

                wx.CallAfter(dialog.finish)  # finish→ID_OK，区分手动取消
    ...
    def should_exclude_channel(self, channel_name: str, data=None) -> bool:
        """检查通道是否应该被排除（keyword + zero + pattern 三级过滤）"""
        ...
        if self.skip_idle_params and '空闲' in channel_name:
            return True
        # 第二级：全零数据过滤（需要 data 参数）
        if self.skip_zero_params and data is not None:
            try:
                if np.max(data) == 0.0 and np.min(data) == 0.0:
                    return True
    ...
    def _process_tdms_data(self, tdms_file, file_name, progress_callback):
        ...
        for group in tdms_file.groups():
            ...
            for channel in group.channels():
                ...
                # 第二级：全零数据过滤
                if self.should_exclude_channel(channel_name, data=data):
                    excluded_count += 1
                    continue
...
class ProgressDialog(wx.Dialog):
    ...
    def finish(self):
        """导入正常完成——关闭对话框（返回 wx.ID_OK 区分用户手动取消）"""
        if self.IsModal():
...
class ImportOptionsDialog(wx.Dialog):
    """导入选项设置对话框"""
    def __init__(self, parent, data_manager):
        super().__init__(parent, title="导入选项", size=(350, 310))
        self.data_manager = data_manager
        self._create_ui()
        self._load_settings()
    ...
    def _create_ui(self):
        ...
        title = wx.StaticText(panel, label="导入时自动跳过以下参数（节省导入时间）：")
        font = title.GetFont()
        font.MakeBold()
        title.SetFont(font)
        sizer.Add(title, 0, wx.ALL, 10)

        self.cb_fault = wx.CheckBox(panel, label='跳过参数名称带"故障"字样的参数')
        self.cb_reserve = wx.CheckBox(panel, label='跳过参数名称带"保留"字样的参数')
        self.cb_standby = wx.CheckBox(panel, label='跳过参数名称带"备用"字样的参数')
        self.cb_idle = wx.CheckBox(panel, label='跳过参数名称带"空闲"字样的参数')
        self.cb_zero = wx.CheckBox(panel, label='跳过最大值和最小值均为 0 的参数（纯零点）')

        sizer.Add(self.cb_fault, 0, wx.ALL, 8)
        sizer.Add(self.cb_reserve, 0, wx.ALL, 8)
        sizer.Add(self.cb_standby, 0, wx.ALL, 8)
        sizer.Add(self.cb_idle, 0, wx.ALL, 8)
        sizer.Add(self.cb_zero, 0, wx.ALL, 8)
    ...
    def _load_settings(self):
        ...
        self.cb_zero.SetValue(self.data_manager.skip_zero_params)
    ...
    def _save_settings(self):
        ...
        self.data_manager.skip_zero_params = self.cb_zero.GetValue()
...
class EffectiveValueDialog(wx.Dialog):
    ...
    def __init__(self, parent, data_manager):
        title = "有效值获取方式"
        super().__init__(parent, title=title, size=(360, 300),
                         style=wx.DEFAULT_DIALOG_STYLE | wx.RESIZE_BORDER)
        self.data_manager = data_manager
        self._create_ui()
    ...
    def GetSelectedMethod(self):
        ...
        choices = list(Config.EFFECTIVE_VALUE_METHODS.keys())
        return choices[idx], Config.EFFECTIVE_VALUE_METHODS[choices[idx]]
...
class ParameterGridPanel(wx.Panel):
    ...
    def _setup_special_columns(self):
        ...
        # 颜色列——渲染器绘制色块，点击由 _on_cell_left_click 处理弹出选择器
        ...
        # 线型列——渲染器绘制线型，点击弹出视觉化选择器
        linestyle_attr = wx.grid.GridCellAttr()
        linestyle_attr.SetRenderer(LineStyleRenderer())
        linestyle_attr.SetReadOnly(True)  # 禁用编辑模式，由 _on_cell_left_click 处理
...
class SignalScapeFrame(wx.Frame):
    ...
    def _create_menu(self):
        ...
        filter_item = tool_menu.Append(wx.ID_ANY, "数据筛选设置", "配置数据筛选规则")
        ...
        self.Bind(wx.EVT_MENU, self._on_filter_settings, filter_item)
    ...
    def _update_filter_status(self):
        """更新筛选状态显示"""
        if self.data_manager.filter_enabled:
            status = f"筛选已启用 (排除模式: {len(self.data_manager.exclude_patterns)})"
        else:
            status = "筛选已禁用"
    ...
    def _create_ui(self):
        ...
        # 主布局
        main_sizer = wx.BoxSizer(wx.VERTICAL)
        main_sizer.Add(self._create_main_toolbar(self), 0, wx.EXPAND)  # 工具栏
    ...
    def _create_menu(self):
        """菜单已由工具栏替代，仅设置空菜单栏保持窗口结构完整"""
        self.SetMenuBar(wx.MenuBar())

    def _create_main_toolbar(self, parent):
        """创建主工具栏——使用系统原生图标，简洁优雅"""
        bar = wx.ToolBar(parent, style=wx.TB_FLAT | wx.TB_HORZ_TEXT)
        bar.SetToolBitmapSize((16, 16))
        bar.SetFont(wx.Font(9, wx.FONTFAMILY_DEFAULT, wx.FONTSTYLE_NORMAL, wx.FONTWEIGHT_NORMAL))

        def add(text, handler, tip, art_id):
            bmp = wx.ArtProvider.GetBitmap(art_id, wx.ART_TOOLBAR, (16, 16))
            tool = bar.AddTool(wx.ID_ANY, text, bmp, tip, wx.ITEM_NORMAL)
            bar.Bind(wx.EVT_TOOL, handler, tool)
            return tool

        add("打开文件", self._on_open, "打开单个TDMS文件", wx.ART_FILE_OPEN)
        add("批量打开", self._on_batch_open, "批量打开多个TDMS文件", wx.ART_FOLDER_OPEN)
        bar.AddSeparator()
        add("有效值计算", self._on_effective_value_settings, "设置有效值计算方法", wx.ART_TIP)
        bar.AddSeparator()
        add("导入选项", self._on_import_options, "配置导入过滤规则", wx.ART_EXECUTABLE_FILE)
        add("数据筛选", self._on_filter_settings, "配置数据筛选规则", wx.ART_CROSS_MARK)
        bar.Realize()
        return bar
    ...
    def _on_effective_value_settings(self, event):
        ...
        if dlg.ShowModal() == wx.ID_OK:
            method_name, window = dlg.GetSelectedMethod()
